...
import { readFile, readdir, stat, lstat, realpath, rename, watch, writeFile, chmod, unlink } from 'fs/promises';
...
let ARCH_DIR = process.env.ARCH_DIR || join(CODEX_HOME, 'arch');
...
async function watchDocs() {
  try {
    for await (const _event of watch(ARCH_DIR, { recursive: true })) broadcastChange();
  } catch (err) {
    console.error('Arch directory watcher stopped:', err.message);
  }
}

if (process.argv[1] === fileURLToPath(import.meta.url)) watchDocs();
...
// Keep target paths inside the architecture directory and prevent ../ escapes.
function safeJoin(...parts) {
  const target = resolve(...parts);
  if (target !== ARCH_DIR && !target.startsWith(ARCH_DIR + sep)) return null;
  return target;
}
...
function documentPath(projectId, date, fileName) {
  ...
  return safeJoin(ARCH_DIR, projectId, date, fileName);
}

const MAX_MARKDOWN_BODY_BYTES = 2 * 1024 * 1024;
...
async function resolveDocumentFile(filePath) {
  ...
  const relativePath = filePath.slice(ARCH_DIR.length + 1);
...
async function writeMarkdownSafely(filePath, content, mode) {
  const tempPath = join(dirname(filePath), `.${randomUUID()}.tmp`);
  try {
    await writeFile(tempPath, content, { encoding: 'utf8', mode: mode & 0o777, flag: 'wx' });
...
// Read images stored next to the selected document.
app.get('/api/assets/:projectId/:date/*assetPath', async (req, res) => {
  try {
    ...
    const filePath = safeJoin(ARCH_DIR, req.params.projectId, req.params.date, assetPath);
...
app.get('/api/projects', async (_req, res) => {
  try {
    const projects = await readdir(ARCH_DIR);
    const result = [];

    for (const project of projects) {
      const projDir = join(ARCH_DIR, project);
      let st;
      ...
      result.push({ id: project, name: decodeProjectName(project), docCount, lastActive });
    }
...
app.get('/api/projects/:projectId/dates', async (req, res) => {
  try {
    const projDir = safeJoin(ARCH_DIR, req.params.projectId);
    ...
    for (const d of entries) {
      ...
      dates.push({ date: d, docCount: documentFiles.length, lastActive });
    }
...
// List architecture documents within a date group.
app.get('/api/projects/:projectId/dates/:date/docs', async (req, res) => {
  try {
    const dateDir = safeJoin(ARCH_DIR, req.params.projectId, req.params.date);
...
// Read an architecture document.
app.get('/api/docs/:projectId/:date/:docId', async (req, res) => {
  try {
    const requestedPath = documentPath(req.params.projectId, req.params.date, req.params.docId);
    ...
    const extension = documentExtension(req.params.docId);
    if (extension === '.md') {
      ...
      return res.json({
        ...
        editable: true,
        name: documentName(req.params.docId),
        extension,
      });
    }
    res.json({ content, editable: false, name: documentName(req.params.docId), extension });
  } catch (err) {
    if (err.code === 'ENOENT') return res.status(404).json({ error: 'doc not found' });
    if (err.code === 'INVALID_DOCUMENT_PATH') return res.status(400).json({ error: err.message });
    res.status(500).json({ error: err.message });
  }
});

// Save the Markdown body of an architecture document.
app.put('/api/docs/:projectId/:date/:docId/content', async (req, res) => {
  try {
    const requestedPath = documentPath(req.params.projectId, req.params.date, req.params.docId);
    if (!requestedPath || documentExtension(req.params.docId) !== '.md') {
      return res.status(400).json({ error: 'only markdown documents are editable' });
    }
    ...
    if (Buffer.byteLength(body, 'utf8') > MAX_MARKDOWN_BODY_BYTES) {
      return res.status(413).json({ error: 'markdown content too large' });
    }
    ...
    await serializeWrite(requestedPath, async () => {
      ...
      if (revisionFor(original) !== revision) {
        return res.status(409).json({ error: 'document changed outside the editor' });
      }

      const { frontmatter } = splitMarkdownFrontmatter(original);
      ...
      if (nextContent !== original) {
        ...
        await writeMarkdownSafely(filePath, nextContent, fileInfo.mode);
      }

      res.json({ content: body, revision: revisionFor(nextContent), editable: true });
    });
  } catch (err) {
    if (err.code === 'ENOENT') return res.status(404).json({ error: 'doc not found' });
    if (err.code === 'INVALID_DOCUMENT_PATH') return res.status(400).json({ error: err.message });
...
// Rename an architecture document file.
app.patch('/api/docs/:projectId/:date/:docId', async (req, res) => {
  try {
    const oldPath = documentPath(req.params.projectId, req.params.date, req.params.docId);
    if (!oldPath) return res.status(400).json({ error: 'invalid path' });

    let st;
    try { st = await stat(oldPath); } catch { return res.status(404).json({ error: 'doc not found' }); }
    ...
    if (!newPath.startsWith(ARCH_DIR)) return res.status(400).json({ error: 'invalid path' });
    if (newPath === oldPath) return res.json({ ok: true, name: newName, id: req.params.docId });
...
// Move an architecture document to the operating system trash.
app.delete('/api/docs/:projectId/:date/:docId', async (req, res) => {
  if (!trashSupported) {
    return res.status(501).json({ error: 'moving to trash is unavailable on this platform' });
  }

  try {
    const filePath = documentPath(req.params.projectId, req.params.date, req.params.docId);
    if (!filePath) return res.status(400).json({ error: 'invalid path' });

    let st;
    try { st = await stat(filePath); } catch { return res.status(404).json({ error: 'doc not found' }); }
    if (!st.isFile()) return res.status(400).json({ error: 'not a file' });
...
export function createApp(options = {}) {
  const directory = typeof options === 'string' ? options : options.archDir;
  if (directory) ARCH_DIR = directory;
  return app;
}

export function createArchServer(options = {}) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  server.listen(PORT, '127.0.0.1', () => {
    console.log(`Arch Viewer: http://127.0.0.1:${PORT}`);
  });
}
