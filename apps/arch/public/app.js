...
const localeMessages = Object.freeze({
  en: Object.freeze({
    appName: 'Arch Viewer',
    selectDocument: 'Select an architecture document to view',
    noDocuments: 'No documents',
    navigationTitle: 'Architecture documents',
    collapseAll: 'Collapse all',
    loading: 'Loading...',
    loadFailed: 'Failed to load',
    none: 'None',
    noDocumentsInDate: 'No documents',
    rename: 'Rename',
    copyPath: 'Copy path',
  ...
  'zh-CN': Object.freeze({
    appName: '架构查看器',
    selectDocument: '选择一个梳理文档查看',
    noDocuments: '暂无梳理文档',
    navigationTitle: '梳理目录',
    collapseAll: '折叠全部目录',
    loading: 'Loading...',
    loadFailed: '加载失败',
    none: '无',
    noDocumentsInDate: '无文档',
    rename: '重命名',
    copyPath: '复制路径',
    moveToTrash: '移到废纸篓',
...
const VIEW_STATE_KEY = 'archViewerState';
...
(function initSidebarWidth() {
  const saved = parseInt(localStorage.getItem('archSidebarWidth') || '', 10);
...
resizerEl.addEventListener('mousedown', (e) => {
  ...
  const onUp = () => {
    ...
    localStorage.setItem('archSidebarWidth', sidebarEl.offsetWidth);
...
function clearAnnotationsForCurrentDoc(path = currentPath) {
  if (path) localStorage.removeItem(ANN_KEY_PREFIX + path);
  annotations = [];
  removeAnnBubble();
  closeAnnPopup();
}

function addEditControl(bodyEl, actions, markdown, revision) {
  const editBtn = actions.querySelector('.doc-edit-btn');
  editBtn.addEventListener('click', () => enterEditMode(markdown, revision));
  ...
  if (firstH1) {
    ...
    label.textContent = t('documentActions');
    actions.prepend(label);
  }
}

function renderMarkdownPreview(markdown, revision, { scrollTop = 0 } = {}) {
  const content = document.getElementById('content');
  const html = renderMarkdown(markdown || '');
  content.innerHTML = `<div class="doc-shell"><div class="document-actions"><button type="button" class="doc-edit-btn" data-i18n="edit">Edit</button></div><div class="doc-body">${html}</div></div>`;
  const bodyEl = content.querySelector('.doc-body');
  ...
  if (bodyEl && actions) {
    ...
    addEditControl(bodyEl, actions, markdown || '', revision);
  }
  content.scrollTop = scrollTop;
  resetAnnotationsForCurrentDoc();
}

function enterEditMode(markdown, revision) {
  ...
  editor.innerHTML = `
    <div class="editor-toolbar">
      <span class="editor-label" data-i18n="editMarkdownBody">Edit Markdown body</span>
      <button type="button" class="cancel-btn" data-i18n="cancel">Cancel</button>
      <button type="button" class="save-btn" data-i18n="save">Save</button>
    </div>
    <div class="editor-shell">
      <div class="editor-line-numbers" aria-hidden="true"></div>
      <textarea class="editor-textarea" wrap="off" spellcheck="false" aria-label="' + t('markdownBody') + '"></textarea>
    </div>`;
  ...
  editorState = {
    ...
    textarea,
    lineNumbers,
    saveBtn: editor.querySelector('.save-btn'),
    cancelBtn: editor.querySelector('.cancel-btn'),
  };
  ...
  editorState.saveBtn.addEventListener('click', saveEditor);
  editorState.cancelBtn.addEventListener('click', cancelEditor);
  textarea.focus();
}

function cancelEditor() {
  if (!editorState || !confirmDiscardChanges()) return;
  const { originalContent, revision } = editorState;
  clearEditorState();
  renderMarkdownPreview(originalContent, revision);
  if (refreshPending) {
...
async function saveEditor() {
  ...
  try {
    const res = await fetch(`/api/docs/${encodeURIComponent(state.projectId)}/${state.date}/${encodeURIComponent(state.docId)}/content`, {
      method: 'PUT',
    ...
    clearAnnotationsForCurrentDoc(state.path);
    clearEditorState();
    renderMarkdownPreview(data.content, data.revision);
    saveViewState();
  } catch (error) {
...
function renderSidebar(projects) {
  ...
  for (const p of projects) {
    ...
    html += `<span class="badge">${p.docCount}</span>`;
    html += `</div>`;
    ...
    if (!list.dataset.loaded) {
      ...
      try {
        ...
        for (const d of dates) {
          ...
          html += `<span class="badge">${d.docCount}</span>`;
          html += `</div>`;
          html += `<ul class="doc-list" id="docs-${escapeHtml(projectId)}-${d.date}"></ul>`;
          html += `</li>`;
        }
...
async function toggleDate(projectId, date, headerEl) {
  ...
  if (wasCollapsed) {
    const list = document.getElementById(`docs-${projectId}-${date}`);
    if (!list.dataset.loaded) {
      list.innerHTML = '<li style="padding:.4rem 2.2rem;font-size:.75rem;color:var(--cloud-medium)">' + t('loading') + '</li>';
      try {
        const res = await fetch(`/api/projects/${encodeURIComponent(projectId)}/dates/${date}/docs`);
        const docs = await res.json();
        let html = '';
        for (const p of docs) {
          const pid = escapeHtml(p.id);
          const pathAttr = escapeHtml(p.path || '');
          html += `<li class="doc-item" data-id="${pid}" data-path="${pathAttr}" onclick="selectDoc('${escapeHtml(projectId)}','${date}','${pid}', this)">`;
          html += `<span class="doc-title">${escapeHtml(p.name)}</span>`;
          html += `<button class="rename-btn" title="${t('rename')}" onclick="renameDoc(event,'${escapeHtml(projectId)}','${date}','${pid}', this)">`;
          html += RENAME_SVG;
          html += `</button>`;
          html += `<button class="copy-btn" title="${t('copyPath')}" onclick="copyPath(event, this)">`;
          html += COPY_SVG;
          html += `</button>`;
          html += `<button class="trash-btn" title="${t('moveToTrash')}" onclick="deleteDoc(event,'${escapeHtml(projectId)}','${date}','${pid}')">`;
          html += TRASH_SVG;
          html += `</button>`;
          html += `</li>`;
        }
...
async function selectDoc(projectId, date, docId, el, { scrollTop = 0, persist = true } = {}) {
  if (editorState?.saving) return;
  if (isEditing() && !confirmDiscardChanges()) return;
  if (isEditing()) clearEditorState();
  const loadGeneration = ++documentLoadGeneration;
  document.querySelectorAll('.doc-item.active').forEach(e => e.classList.remove('active'));
  ...
  try {
    const res = await fetch(`/api/docs/${encodeURIComponent(projectId)}/${date}/${encodeURIComponent(docId)}`);
    if (!res.ok) throw new Error('doc not found');
    const data = await res.json();
    if (loadGeneration !== documentLoadGeneration || currentProjectId !== projectId || currentDate !== date || currentDocId !== docId) return;
    if (data.extension === '.html') {
      ...
      renderMarkdownPreview(data.content || '', data.revision, { scrollTop });
    }
...
async function renameDoc(event, projectId, date, docId, btn) {
  ...
  const titleSpan = li.querySelector('.doc-title');
  ...
  let finish = async (commit) => {
    ...
    if (!commit || !newName || newName === oldName) {
      ...
      li.onclick = () => selectDoc(projectId, date, docId, li);
      return;
    }
    try {
      const res = await fetch(`/api/docs/${encodeURIComponent(projectId)}/${date}/${encodeURIComponent(docId)}`, {
      ...
      if (data.ok) {
        ...
        if (oldPath) {
          ...
          if (oldData) {
            const extension = docId.slice(docId.lastIndexOf('.'));
        ...
        const renamedId = data.id || newName + docId.slice(docId.lastIndexOf('.'));
        const newLi = document.querySelector(`#docs-${projectId}-${date} .doc-item[data-id="${CSS.escape(renamedId)}"]`);
        if (newLi) selectDoc(projectId, date, renamedId, newLi);
      } else {
...
async function restoreViewState() {
  ...
  if (state.projectId && state.date && state.docId) {
    const item = document.querySelector(`#docs-${CSS.escape(state.projectId)}-${CSS.escape(state.date)} .doc-item[data-id="${CSS.escape(state.docId)}"]`);
    if (item) await selectDoc(state.projectId, state.date, state.docId, item, { scrollTop: state.scrollTop || 0, persist: false });
    else {
      currentProjectId = null;
      currentDate = null;
      currentDocId = null;
      document.getElementById('content').innerHTML = '<div class="empty"><p data-i18n="selectDocument">Select an architecture document to view</p></div>';
    }
  }
...
async function refreshDateGroup(projectId, date) {
  ...
  for (const p of docs) {
    ...
    html += `<li class="doc-item" data-id="${pid}" data-path="${pathAttr}" onclick="selectDoc('${escapeHtml(projectId)}','${date}','${pid}', this)">`;
    html += `<span class="doc-title">${escapeHtml(p.name)}</span>`;
    html += `<button class="rename-btn" title="${t('rename')}" onclick="renameDoc(event,'${escapeHtml(projectId)}','${date}','${pid}', this)">${RENAME_SVG}</button>`;
    html += `<button class="copy-btn" title="${t('copyPath')}" onclick="copyPath(event, this)">${COPY_SVG}</button>`;
    html += `<button class="trash-btn" title="${t('moveToTrash')}" onclick="deleteDoc(event,'${escapeHtml(projectId)}','${date}','${pid}')">${TRASH_SVG}</button>`;
    html += `</li>`;
  }
  list.innerHTML = html || '<li style="padding:.4rem 2.2rem;font-size:.75rem;color:var(--cloud-medium)">' + t('noDocumentsInDate') + '</li>';
}
...
async function deleteDoc(event, projectId, date, docId) {
  ...
  if (!confirm(t('confirmTrash') + docId)) return;
  // Read the item path so its annotations can be removed.
  const li = event.currentTarget?.closest?.('.doc-item');
  const filePath = li?.dataset?.path;
  try {
    const res = await fetch(`/api/docs/${encodeURIComponent(projectId)}/${date}/${encodeURIComponent(docId)}`, { method: 'DELETE' });
    const data = await res.json();
    if (data.ok) {
      ...
      if (currentProjectId === projectId && currentDate === date && currentDocId === docId) {
        document.getElementById('content').innerHTML = '<div class="empty"><p data-i18n="selectDocument">Select an architecture document to view</p></div>';
        currentDocId = null;
        currentPath = null;
        annotations = [];
      ...
      const docs = await (await fetch(`/api/projects/${encodeURIComponent(projectId)}/dates/${date}/docs`)).json();
      if (docs.length === 0) {
...
// Annotation support.
const ANN_KEY_PREFIX = 'archAnnotations:';
