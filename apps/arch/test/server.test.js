...
test('reads, updates, rejects stale writes, renames, and blocks unsafe paths', async () => {
  const fixture = await startFixture();
  try {
    const loadedResponse = await fetch(`${fixture.base}/api/docs/project/2026-01-01/Document.md`);
    assert.equal(loadedResponse.status, 200);
    const loaded = await loadedResponse.json();
    assert.equal(loaded.content, '# Document\n');
    assert.match(loaded.revision, /^[a-f0-9]{64}$/);

    const updatedResponse = await fetch(`${fixture.base}/api/docs/project/2026-01-01/Document.md/content`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: '# Updated\n', revision: loaded.revision }),
    });
    assert.equal(updatedResponse.status, 200);
    const staleResponse = await fetch(`${fixture.base}/api/docs/project/2026-01-01/Document.md/content`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: '# Stale\n', revision: loaded.revision }),
    });
    assert.equal(staleResponse.status, 409);

    const renameResponse = await fetch(`${fixture.base}/api/docs/project/2026-01-01/Document.md`, {
    ...
    const traversal = await fetch(`${fixture.base}/api/docs/project/2026-01-01/${encodeURIComponent('../outside.md')}`);
    ...
    const linked = await fetch(`${fixture.base}/api/docs/project/2026-01-01/Linked.md`);
